...
def test_bs4_is_middle_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(trafilatura, "extract", lambda *_a, **_k: "too short")
...
def test_use_browser_routes_to_rendered_html(tmp_path: Path) -> None:
...
def test_use_browser_false_uses_fetch_url(tmp_path: Path) -> None:
    urls_file = tmp_path / "urls.txt"
    urls_file.write_text("https://example.com/article\n", encoding="utf-8")
    repo = MagicMock()
    repo.categorise.return_value = "article"
